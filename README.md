...
## ファイル形式の違い
### MessagePack(非圧縮)
比べてファイルサイズが小さく、読み書きが非常に高速です。

### MessagePack(LZ4圧縮)
上記のMessagePackに高速なLZ4圧縮を加えるのでオーバーヘッドを少なくした状態で容量を小さくすることができます。

### Json
ファイルサイズが大きくなりますが、比較的簡単に読み込むことができます。

### CSV
通常のカンマ区切りのCSVです。ファイルサイズが少し大きくなりますが、どのアプリケーションでも簡単に処理することができます。

## 保存したファイルの使用方法
pbf形式にて保存したファイルを読み込む際には`KyoshinMonitorLib`のクラスを使用してください。(.protoかけてなくてスミマセン！)

csv形式で保存したファイルの出力形式は、  
```
所属しているネットワークの種類(0=不明,1=KiK-net,2=K-NET),地点コード,休止(無効)状態にあるかどうか(True/False),観測地点名,観測地点が属する地域の名称,緯度,経度,強震モニタ上でのX座標,強震モニタ上でのY座標,緊急地震速報や震度速報で用いる区域のID,緊急地震速報で用いる府県予報区のID,日本座標系の緯度,日本座標系の経度
```
...
## 再配布の手引
このソフトウェア自体の再配布は、**各ライブラリのライセンスに従う**、**LICENSE.mdを添付する**の2点を守っていただければ問題ありません。
できれば報告もしていただけると助かります。  
自作ソフトと配布する場合、設定ファイルを書き換え、そのソフトが使用しているファイルを起動時に読み込むようにしてやってもいいかもしれません。